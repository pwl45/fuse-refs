...
static int initialize_reg_inode(int parent_inum, mode_t mode) {
	...
	inode_table[inum].inode.mode = S_IFREG | mode; //NOTE: DO WE NEED TO 'OR' this with the 'mode' parameter
...
int refs_access(const char* path, int mask){
	//TODO: Implement actual permission checking; add mode field to inode
/*
...
int refs_mkdir(const char* path, mode_t mode){
	//char testpath[] = "/foo/bar/baz/test";
	//TODO: loop through block ptrs?
	char* pathdup = strdup(path);
	int child_inum, parent_inum;
	resolve_path(path, &parent_inum, &child_inum);
	...
	if(block_count == child_inode->blocks){
		...
		while(child_inode->blocks < block_count){
			if(child_inode->blocks < NUM_DIRECT){
				...
				lba_t lba = reserve_data_block();
				child_inode->block_ptrs[child_inode->blocks] = lba;
				lba_t indirectlba = reserve_data_block();
				lba_t* indirect_block = calloc_blocks(1);
				indirect_block[0] = indirectlba;
				write_block(indirect_block,lba);
			}
		}
		free(empty_block);
		child_inode->size = size;
...
/* 		} */
/* 		else if(remaining_size > 0){ */
/* 			return copy_size + refs_write(path, buf + copy_size, remaining_size, offset + copy_size, fi); */
/* 		} */
/* 		else { */
/* 			printf("Error: This should never be reached. Math error somewhere"); */
/* 			return -ENOSYS; */
...
/* 		} */
/* 		else if(remaining_size > 0){ */
/* 			return copy_size + refs_read(path, buf + copy_size, remaining_size, offset + copy_size, fi); */
/* 		} */
/* 		else { */
/* 			printf("Error: This should never be reached. Math error somewhere"); */
/* 			return -ENOSYS; */
...
//first: allocate data block store first chunk of direct pointers
// inside that allocate data b  ocks
//Write to block a lba_t*
//Keep track of number of blocks in inode to know how far the lba_t* goes
/* TODO: FREE ALL PATHDUPS */
#define FUSE_USE_VERSION 26
